...
- **Usage Example**:
  ...
  result = collector.fetch_lst_aoi(bbox, "2026-06-01", "2026-08-31", max_cloud_cover=15.0)
...
- **Key Classes & Data Structures**:
  ...
  - `SentinelLULCCollector`: Fetches multispectral composites and categorical LULC rasters.
- **Usage Example**:
  ```python
  ...
  result = collector.fetch_multispectral_and_lulc((-74.02, 40.70, -73.95, 40.78), "2026-06-01", "2026-08-31")
...
- **Role**: Ingests hourly atmospheric boundary conditions from ECMWF ERA5-Land reanalysis.
- **Key Classes & Data Structures**:
...
- **Usage Example**:
  ...
  meteo = collector.fetch_hourly_meteo((-74.02, 40.70, -73.95, 40.78), datetime(2026, 7, 15, 14, 0))
...
- **Key Classes & Data Structures**:
  ...
  - `OSMMorphologyCollector`: Parses building storey counts (`building:levels` $\times 3.2\text{m}$) and heights, rasterizing polygons to regular computational grids.
...
- **Usage Example**:
  ...
  morph = collector.fetch_morphology((-74.02, 40.70, -73.95, 40.78), grid_shape=(64, 64))
